...
def solve(f0, f1, u, v, dt, num_steps, points, n_period, mesh, incident):
    ...
    # u_quad = u0.eval(points.T, cells)[:, 0]

    # u_on_axis = np.zeros([points.T.shape[0], n_period])
    u_quad = np.zeros(points.T.shape[0])
    ...
    for step in range(num_steps):
        ...
        for i in range(n_RK):
            ...
            # Update solution
            # with Timer("SAM: axpy operations"):
        ...
        if step >= num_steps - n_period:
            ...
            incident.t = t
            inc_temp = incident.eval(points)
            u_quad += (evaluate(points, mesh, u) + inc_temp)**2

        # print('RK time for one step = ', end - start)
        # timer.stop()
    end = time.time()
