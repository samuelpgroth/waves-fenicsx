...
import time
from meshing import build_piston
import matplotlib
...
class WesterveltEquation:
    def __init__(self, mesh, k, omega, c, wx, wy, amp):
        ...
        # FIMXE: I think we only need facets0
        ...
        # facets2 belong to the curved boundary of the cylinder
        facets2 = locate_entities_boundary(mesh, tdim - 1,
                                           lambda x: np.sqrt(x[1]**2 + x[2]**2)
                                           > (wy / 2 - 1.0e-6))
...
# Build mesh
mesh = build_piston(nx, l=wx, r=wy/2)
